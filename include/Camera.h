...
public:
	...
	explicit Camera(float const &dist, Vec3f const &pos = Vec3f(), Vec3f const &forward = Vec3f(),
		Vec3f const &up = Vec3f());
...
inline Camera::Camera(float const &dist, Vec3f const &pos, Vec3f const &forward, Vec3f const &up)
	: m_focusDist(dist), m_pos(pos), m_up(up), m_forward(forward) {}
